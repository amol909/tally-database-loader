...
export function validateConfig(config: appConfig): string[] {
    ...
    if (!config.database) errors.push('Missing "database" section.');
    if (!config.tally) errors.push('Missing "tally" section.');
    ...
    if (!config.database.schema) errors.push('database.schema is required.');
    if (typeof config.database.username !== 'string') errors.push('database.username must be a string.');
    if (typeof config.database.password !== 'string') errors.push('database.password must be a string.');
    ...
    if (typeof config.database.ssl !== 'boolean') errors.push('database.ssl must be true or false.');

    if (!config.tally.definition) errors.push('tally.definition is required.');
    if (!config.tally.server) errors.push('tally.server is required.');
    if (!Number.isInteger(config.tally.port) || config.tally.port <= 0) {
        errors.push('tally.port must be a positive integer.');
    }
    for (const fieldName of ['fromdate', 'todate'] as const) {
    ...
    if (typeof config.tally.company !== 'string') errors.push('tally.company must be a string.');
