...
export function buildVoucherInventoryLinesRequest(config, options = {}) {
...
export function parseVoucherInventoryRows(xml) {
...
export function resolveVoucherInventoryMarkerAlterId(rows, toAlterId = 0) {
...
export function resolveVoucherInventoryUpdateMarker(options) {
    if (options.updateMarker !== undefined) {
        return options.updateMarker;
    }
    return !options.noUpdateMarker;
}
export function buildVoucherInventoryIncrementalOptions(fromAlterId, toAlterId) {
...
export async function readVoucherInventoryStartAlterId() {
...
export async function readLastTransactionAlterId() {
    if (database.config.technology != 'postgres') {
        return 0;
    }
    try {
...
export async function replaceVoucherInventoryRows(rows, updateMarker = true, markerAlterId = null) {
    ...
    try {
        ...
        for (let index = 0; index < rows.length; index += batchSize) {
            ...
            const values = batch.map((row, rowIndex) => {
                ...
                params.push(row.guid, row.item, row.itemGuid, row.quantity, row.rate, row.amount, row.additionalAmount, row.discountAmount, row.godown, row.godownGuid, row.trackingNumber, row.orderNumber, row.orderDueDate, row.alterId);
            ...
            await client.query(`insert into _trn_inventory_stage (
...
export async function refreshVoucherInventoryLines(config, options = {}) {
    if (database.config.technology != 'postgres') {
...
//# sourceMappingURL=voucher-inventory-lines.mjs.map